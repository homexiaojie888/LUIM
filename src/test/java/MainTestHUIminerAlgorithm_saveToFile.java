...
public class MainTestHUIminerAlgorithm_saveToFile {
...
//		String input6 = "BMSPHM.txt";
		...
		String output = ".//outputHuiminer.txt";
...
//		int[] max_utility_retail = new int[]{10,20,30,40,50,60};
		...
		for (int i = 0; i < max_utility.length; i++) {
			...
			HUIMiner huiMiner=new HUIMiner();
			huiMiner.runAlgorithm(input,output,max_utility[i]);
			MemoryLogger.getInstance().checkMemory();
			huiMiner.printStats(runTime,memory,candidates,pattern);
		}
		OutputExp(max_utility,finalInput);

	}
	private static void OutputExp(int[] max_utility, String input) throws IOException {
		String experimentFile = ".//exp_huiminer_"+input;
	...
	public static String fileToPath(String filename) throws UnsupportedEncodingException{
		URL url = MainTestHUIminerAlgorithm_saveToFile.class.getResource(filename);
